...
// replay_serve.go implements `agentry replay-serve <fixture>` — the thin compose-container
...
This is the container that ` + "`agentry dev --replay`" + ` swaps in for the mock
...
func runReplayServe(ctx context.Context, fixturePath string, port int, out, errOut interface {
	...
	statusf(out, "agentry replay-serve: run=%q agent=%q model=%d tool(s)=%d — serving on :%d\n",
	...
	case <-runCtx.Done():
		...
		statusf(errOut, "agentry replay-serve: shut down\n")
		return nil
	}
}
