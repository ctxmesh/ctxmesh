...
// dev.go implements `agentry dev` — the local inner-loop command (spec §22).
...
Examples:
  agentry dev -f examples/echo-agent/agent.yaml
  agentry dev -f agent.yaml --port 9090
  agentry dev -f agent.yaml --provider real \
...
func runDev(ctx context.Context, fv devFlagValues, out, errOut io.Writer) error {
	...
	statusf(out, "agentry dev: starting stack for %q (provider=%s, route=%s)\n",
	...
	teardown := func() {
		...
		if derr := down.Run(); derr != nil {
			statusf(errOut, "agentry dev: teardown warning: %v\n", derr)
		}
	}
	...
	if fv.noWait {
		statusf(out, "agentry dev: stack up (--no-wait); /invoke at %s\n", plan.InvokeURL())
...
func serveDevUI(uiPort int, uiDist string, plan *devPlan, out, errOut io.Writer) (func(), error) {
	...
	go func() {
		...
		if err != nil && !errors.Is(err, http.ErrServerClosed) {
			statusf(errOut, "agentry dev: console UI server error: %v\n", err)
			lisErr <- err
			return
		}
		lisErr <- nil
	...
	statusf(out, "\nagentry dev: console UI at http://%s\n", addr)
...
func writePlanAssets(plan *devPlan) (string, func(), error) {
	dir, err := os.MkdirTemp("", "agentry-dev-")
	if err != nil {
...
func waitForInvoke(ctx context.Context, plan *devPlan, out, errOut io.Writer) error {
	...
	statusf(out, "agentry dev: waiting for %s to answer…\n", plan.InvokeURL())
	...
	for time.Now().Before(deadline) {
		...
		if err != nil {
			...
			statusf(out, "agentry dev: /invoke answered: %s\n", truncate(body, 200))
			return nil
		}
	...
	statusf(errOut, "agentry dev: agent did not become ready in %s\n", devReadyTimeout)
