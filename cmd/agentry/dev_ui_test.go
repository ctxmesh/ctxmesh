// dev_ui_test.go covers `agentry dev --ui` (ADR 0021): the local dev-mode
...
func TestServeDevUI_ServesConsoleAndDegrades(t *testing.T) {
	dist := t.TempDir()
	const marker = "<!doctype html><title>agentry console</title>"
	...
	assert.Equal(t, http.StatusOK, code)
	assert.Contains(t, body, "agentry console")
}
