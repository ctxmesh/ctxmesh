...
// dev_plan.go holds the PURE planning logic for `agentry dev`: it turns a
...
//
// `agentry dev` renders a Docker Compose stack — the lightest inner loop
...
const (
	...
	// mock) — kept as a literal here so cmd/agentry has no dependency on the
	// controller package, with a unit test asserting they match.
	mockResponseText = "MOCK_OK deterministic response from agentry gateway"
...
// agentry CLI, so this is a new but pattern-consistent image.
func replayImageRef() string { return "agentry-replay:" + devVersion }
...
func renderCompose(p *devPlan) string {
	...
	b.WriteString("# Rendered by `agentry dev` — the local inner loop (spec §22).\n")
