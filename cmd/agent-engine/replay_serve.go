...
// replay_serve.go implements `agent-engine replay-serve <fixture>` — the thin compose-container
...
This is the container that ` + "`agent-engine dev --replay`" + ` swaps in for the mock
...
func runReplayServe(ctx context.Context, fixturePath string, port int, out, errOut interface {
	...
	statusf(out, "agent-engine replay-serve: run=%q agent=%q model=%d tool(s)=%d — serving on :%d\n",
	...
	case <-runCtx.Done():
		...
		statusf(errOut, "agent-engine replay-serve: shut down\n")
		return nil
	}
}
