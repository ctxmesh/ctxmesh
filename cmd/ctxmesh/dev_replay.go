...
func runReplay(
	...
	statusf(out, "ctxmesh dev --replay: run=%q agent=%q — %d model + %d tool interaction(s)\n",
	...
	teardown := func() {
		statusf(out, "\nctxmesh dev --replay: tearing down…\n")
		...
		if derr := down.Run(); derr != nil {
			statusf(errOut, "ctxmesh dev --replay: teardown warning: %v\n", derr)
		}
	}
	defer teardown()
...
func verifyReplayParity(ctx context.Context, out, errOut io.Writer) error {
	...
	statusf(out, "ctxmesh dev --replay: waiting for the replay server…\n")
	...
	for time.Now().Before(deadline) {
		...
		if err != nil {
			...
			if derr != nil {
				...
				statusf(out, "ctxmesh dev --replay: replay server ready (version %s, schema %d)\n",
	...
	statusf(errOut, "ctxmesh dev --replay: replay server did not become ready in %s\n", devReadyTimeout)
