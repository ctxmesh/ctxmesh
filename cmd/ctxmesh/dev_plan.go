...
// dev_plan.go holds the PURE planning logic for `ctxmesh dev`: it turns a
...
//
// `ctxmesh dev` renders a Docker Compose stack — the lightest inner loop
...
const (
	...
	// mock) — kept as a literal here so cmd/ctxmesh has no dependency on the
	// controller package, with a unit test asserting they match.
	mockResponseText = "MOCK_OK deterministic response from ctxmesh gateway"
...
// ctxmesh CLI, so this is a new but pattern-consistent image.
func replayImageRef() string { return "ctxmesh-replay:" + devVersion }
...
func renderCompose(p *devPlan) string {
	...
	b.WriteString("# Rendered by `ctxmesh dev` — the local inner loop (spec §22).\n")
