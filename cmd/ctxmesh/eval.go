...
// eval.go implements `ctxmesh eval` — the out-of-cluster CI/CD gate command
...
// EvalReport is the stable output schema emitted by `ctxmesh eval`.
...
Examples:
  ...
  ctxmesh eval --candidate agent.yaml --min-score 0.75 --output junit
  ctxmesh eval --candidate agent.yaml --min-score 0.90 \
...
func applyObject(ctx context.Context, k8sClient client.Client, errOut io.Writer, obj client.Object) error {
	...
	_, _ = fmt.Fprintf(errOut,
		"ctxmesh eval: %T %q already exists in %q — recreating\n",
...
func pollGateStatus(
	...
	_, _ = fmt.Fprintf(errOut, "ctxmesh eval: polling gate status for %s/%s…\n", namespace, name)
	...
	for {
		...
		case <-ticker.C:
			...
			if err := k8sClient.Get(ctx, types.NamespacedName{Namespace: namespace, Name: name}, &current); err != nil {
				_, _ = fmt.Fprintf(errOut, "ctxmesh eval: get %s: %v (retrying…)\n", name, err)
				continue
			}
			gate := current.Status.Gate
			if gate == nil {
				_, _ = fmt.Fprintf(errOut, "ctxmesh eval: status.gate not yet set (phase: pending)\n")
				continue
			}
			_, _ = fmt.Fprintf(errOut, "ctxmesh eval: gate phase=%s score=%s\n", gate.Phase, gate.Score)
...
func teardownEvalResources(
	...
	if ad != nil {
		if err := k8sClient.Delete(ctx, ad); err != nil && !apierrors.IsNotFound(err) {
			_, _ = fmt.Fprintf(errOut,
				"ctxmesh eval: teardown warning: deleting AgentDeployment %q: %v\n", ad.Name, err)
		}
	}
	if es != nil {
		if err := k8sClient.Delete(ctx, es); err != nil && !apierrors.IsNotFound(err) {
			_, _ = fmt.Fprintf(errOut,
				"ctxmesh eval: teardown warning: deleting EvalSuite %q: %v\n", es.Name, err)
		}
	}
}
