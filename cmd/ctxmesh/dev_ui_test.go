// dev_ui_test.go covers `ctxmesh dev --ui` (ADR 0021): the local dev-mode
...
func TestServeDevUI_ServesConsoleAndDegrades(t *testing.T) {
	dist := t.TempDir()
	const marker = "<!doctype html><title>ctxmesh console</title>"
	...
	assert.Equal(t, http.StatusOK, code)
	assert.Contains(t, body, "ctxmesh console")
}
