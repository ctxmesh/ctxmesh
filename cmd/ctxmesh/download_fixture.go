...
// download_fixture.go implements `ctxmesh download-fixture <run-id>` — the DX convenience that
...
Examples:
  ctxmesh download-fixture run-abc123
  ctxmesh download-fixture run-abc123 -o /tmp/run.fixture.json
...
func downloadFixtureToFile(
	...
	statusf(out, "ctxmesh download-fixture: run=%q agent=%q — %d model + %d tool interaction(s)\n",
		fx.RunID, fx.Agent, len(fx.Model), len(fx.Tools))
	statusf(out, "  wrote %s (0600 — sensitive; do not commit)\n", outPath)
	statusf(out, "  replay it: ctxmesh dev --replay %s\n", outPath)
	return nil
}
