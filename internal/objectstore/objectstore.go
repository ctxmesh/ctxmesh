...
const (
	...
	// Distinct from the launcher's "ctxmesh-blobs" ephemeral bucket — the two
	// must never share a bucket so their lifecycle policies (GC vs never-GC) cannot
	// bleed across.
	knowledgeBaseBucket = "ctxmesh-knowledge"
...
//   - OBJECT_STORE_ADDR → host:port (e.g. ctxmesh-objectstore.ctxmesh.svc:9000)
