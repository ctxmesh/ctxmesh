...
func TestMetrics_RunOutcomesAndDuration(t *testing.T) {
	...
	assert.Contains(t, body, `ctxmesh_run_outcomes_total{outcome="succeeded"} 2`)
	assert.Contains(t, body, `ctxmesh_run_outcomes_total{outcome="failed"} 1`)
	assert.Contains(t, body, `ctxmesh_run_outcomes_total{outcome="cancelled"} 1`,
	...
	assert.Contains(t, body, `ctxmesh_run_duration_seconds_bucket{outcome="succeeded"`)
	assert.Contains(t, body, `ctxmesh_run_duration_seconds_count{outcome="succeeded"} 2`)
}
...
func TestMetrics_OutcomeCardinalityBounded(t *testing.T) {
	...
	body := scrape(t, m)
	assert.Contains(t, body, `ctxmesh_run_outcomes_total{outcome="unknown"} 1`)
...
func TestMetrics_WorkerActiveGauge(t *testing.T) {
	...
	m.decWorkerActive()
	body := scrape(t, m)
	assert.Contains(t, body, "ctxmesh_run_worker_active 1")
}

func TestMetrics_QueueDepth_FromStore(t *testing.T) {
	m := newMetrics(fakeQueued{n: 7})
	body := scrape(t, m)
	assert.Contains(t, body, "ctxmesh_run_queue_depth 7")
}
...
func TestMetrics_QueueDepth_OmittedOnError(t *testing.T) {
	...
	body := scrape(t, m)
	assert.NotContains(t, body, "ctxmesh_run_queue_depth ")
}
...
func TestMetrics_QueueDepth_AbsentWhenStoreLacksCapability(t *testing.T) {
	...
	m := newMetrics(nil)
	body := scrape(t, m)
	assert.False(t, strings.Contains(body, "ctxmesh_run_queue_depth"),
