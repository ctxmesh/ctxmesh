...
// forToken builds the caller-scoped client from a raw bearer token. It copies the
// in-cluster config and replaces ONLY the credential:
//   - BearerToken is set to the caller's token, and
//   - BearerTokenFile is CLEARED.
//
...
func (f *restCallerClientFactory) forToken(token string) (client.Client, error) {
	...
	return f.newClient(cfg, client.Options{Scheme: f.scheme})
}
