...
func TestBuildTenantLabelVWC_Guards(t *testing.T) {
	vwc := buildTenantLabelVWC("agent-engine-system", "webhook-service", []byte("CA"))
	...
	assert.Equal(t, "agent-engine-system", w.ClientConfig.Service.Namespace)
	...
	assert.Contains(t, exempt.Values, "agent-engine-system", "the install namespace must be exempt (no self-wedge)")
...
func TestBuildTenantLabelVWC_ExemptionTracksInstallNamespace(t *testing.T) {
	...
	assert.NotContains(t, exempt.Values, "agent-engine-system")
}
