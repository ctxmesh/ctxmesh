...
func TestPostgresStore_ListWaitingApproval(t *testing.T) {
	...
	got, err := s.ListWaitingApproval(ctx, "ns-a", []ActionKind{ActionPlanApproval}, 0) // 0 = unbounded
	require.NoError(t, err)
	...
	empty, err := s.ListWaitingApproval(ctx, "ns-empty", []ActionKind{ActionPlanApproval}, 0)
	require.NoError(t, err)
