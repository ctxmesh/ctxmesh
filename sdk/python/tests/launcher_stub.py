"""A lightweight fake of the launcher localhost plane for contract tests.

Three tiny ``http.server`` handlers stand in for the real launcher endpoints so
the SDK clients can be exercised without a live launcher/cluster (tier0):

    MemoryStub    -> the :2998 M5 contract (get/put/append/search)
    DiscoveryStub -> the :2999 M4 contract (/tools) + a fake MCP tool endpoint
    FeedbackStub  -> the :2995 M9 contract (POST /feedback, 202/400/502)

Each stub records the requests it received (method, path, body, headers) so a
test can assert the client hit the right endpoint with the right method/path/
body and the run context (conversationId header, agent identity). Stubs bind an
ephemeral port on localhost and expose ``base_url``.
"""
