...
type KnowledgeBaseSpec struct {
	...
	// hash, the ADR 0045 subject mandate). v1 gates per-user INGESTION off (org-wide corpora
	// only); the field + the knowledge_chunks.subject column exist from day one so per-user
	// ingestion can be enabled without a schema migration. Defaults to false.
	// +optional
